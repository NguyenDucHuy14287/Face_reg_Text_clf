...
class Image_binary_classify_keras(object):
    ...
    @staticmethod
    def getInstance():
        if Image_binary_classify_keras.__instance == None:
            Image_binary_classify_keras()
        return Image_binary_classify_keras.__instance
    ...
    def __init__(self):
        ...
        self.onehot = None
        Image_binary_classify_keras.__instance = self
    ...
    def generate_train_valid(self, img_path_arr, label_arr):
        ...
        # Load image to array
        print(f'===== Load images...')
        train_data = np.array([img_to_array(load_img(img, target_size=(299, 299))) for img in tqdm_notebook(img_path_arr.values.tolist())]).astype('float32')
    ...
    def train_model(self, num_epochs=10, steps_per_epoch=512, validation_steps=32):
    ...
    def predict_video_frames(self, frames_folder_path):
        ...
        # Creae the x_test
        x_test = []
        for image_name in tqdm(frame_lst):
            img = cv2.imread(frames_folder_path + '/' + image_name)
